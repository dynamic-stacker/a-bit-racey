...
class Database:
    ...
    def setup(self):
        ...
        race_timings = "CREATE TABLE IF NOT EXISTS Timings (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(255), record INTEGER);"
        admin_timings = "INSERT OR IGNORE INTO Timings (id, name, record) VALUES (1,'admin',0);"
    ...
    def get_record(self, player_id):
        stmt = "SELECT record FROM Timings WHERE id = ?"
        args = (player_id, )
        self.cursor.execute(stmt, args)
        return self.cursor.fetchone()[0]

    def update_record(self, player_id, record):
        stmt = "UPDATE Timings SET record = ? WHERE id = ? "
        args = (record, player_id, )
        self.cursor.execute(stmt, args)
        self.conn.commit()
